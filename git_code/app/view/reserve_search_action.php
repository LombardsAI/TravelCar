...
//SELECT aeroport, p.label, prix_du_jour, adresse FROM parking p, (SELECT count(*) as num, label_du_parking as label FROM gare g WHERE unix_timestamp(g.date_debut)< unix_timestamp('06/08/2019 00:00:00') OR unix_timestamp(g.date_fin) > unix_timestamp('06/09/2019 00:00:00') GROUP BY label_du_parking)a WHERE (p.aeroport ='CDG' AND p.nombre_max > a.num) OR (p.label NOT IN (a.label))

//SELECT aeroport, p.label, adresse FROM parking p, (SELECT label_du_parking as label,n_plaque FROM gare g WHERE unix_timestamp(g.date_debut)< unix_timestamp('06/08/2019 00:00:00') OR unix_timestamp(g.date_fin)> unix_timestamp('06/09/2019 00:00:00'))a WHERE p.aeroport ='CDG' AND p.label = a.label AND a.n_plaque NOT IN (SELECT e.n_plaque FROM emprunte e WHERE unix_timestamp(e.date_fin) > unix_timestamp('06/09/2019 00:00:00') OR  unix_timestamp(e.date_debut) < unix_timestamp('06/08/2019 00:00:00'))